...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.ignore_index is not None:
            ...
            valid_mask = torch.ones_like(target).unsqueeze(1)

        target_clamped = target.clone()
        if self.ignore_index is not None:
            target_clamped[target == self.ignore_index] = 0

        target_one_hot = F.one_hot(target_clamped, num_classes)
        target_one_hot = target_one_hot.permute(0, 3, 1, 2).float()

        pred = pred * valid_mask
        target_one_hot = target_one_hot * valid_mask

        pred = pred.view(pred.shape[0], num_classes, -1)
        target_one_hot = target_one_hot.view(target_one_hot.shape[0], num_classes, -1)
        ...
        valid_classes = target_one_hot.sum(dim=2) > 0
        dice = dice * valid_classes

        dice_loss = 1 - dice.sum() / valid_classes.sum().clamp_min(1)
        return dice_loss
...
class CrossEntropyLoss(nn.Module):
    """Wrapper for PyTorch CrossEntropyLoss with optional label smoothing and ignore_index."""
